...
To make the bar fancy, most configurations are highly dependent on extra icons, so make sure you install one of [[https://github.com/polybar/polybar/wiki/Fonts#icon-fonts][popular icon fonts]].
...
The following is a very basic polybar configuration of real which defines a bar named =mybar= with a module called =date=:
